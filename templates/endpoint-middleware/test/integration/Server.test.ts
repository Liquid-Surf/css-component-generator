...
describe('My Server', (): void => {
  ...
  it('shows hello world.', async(): Promise<void> => {
    const response = await fetch('http://localhost:3000/template-name', {
      headers: {
        'Accept': 'text/html'
      }
    });
  ...
  it('shows hello + name.', async(): Promise<void> => {
    const response = await fetch('http://localhost:3000/template-name?name=John');
    expect(response.status).toBe(200);
    expect(await response.text()).toBe('<h1> hello John </h1>');
  });
});
